 ...
 *    2. EVICT       - conflict stores to set 0, forcing evictions
 ...
 *   [6..13] SPM_Nw  - for N = 1..8:
 *       SPM_WRITE_N - direct SRAM writes to N scratchpad banks (ways 0..N-1)
 *       SPM_READ_N  - direct SRAM reads from N scratchpad banks
 *   14. IDLE        - no operations, clock free-running
 */
...
module testbench();
  ...
  localparam num_spm_ops_lp          = 8192;   // per-round ops (shorter, 8 rounds)
  ...
  bsg_nonsynth_clock_gen #(.cycle_time_p(20)) clk_gen (.o(clk));
  bsg_nonsynth_reset_gen #(.reset_cycles_lo_p(0),.reset_cycles_hi_p(10)) reset_gen (
    .clk_i(clk), .async_reset_o(reset)
  );
  ...
  bsg_cache_l2_spm DUT (
    ...
    ,.sp_en_i(sp_en), .sp_v_i(sp_v), .sp_w_i(sp_w)
    ,.sp_addr_i(sp_addr), .sp_data_i(sp_wdata), .sp_w_mask_i(sp_w_mask), .sp_data_o(sp_rdata)
  );
  ...
  bsg_nonsynth_dma_model #(
    ...
    ,.mask_width_p(block_size_in_words_p), .block_size_in_words_p(block_size_in_words_p)
    ,.els_p(dma_els_lp)
    ,.read_delay_p(0), .write_delay_p(0), .dma_req_delay_p(0), .dma_data_delay_p(0)
  ) dma (
    .clk_i(clk), .reset_i(reset)
    ,.dma_pkt_i(dma_pkt), .dma_pkt_v_i(dma_pkt_v_lo), .dma_pkt_yumi_o(dma_pkt_yumi_li)
    ,.dma_data_o(dma_data_li), .dma_data_v_o(dma_data_v_li), .dma_data_ready_i(dma_data_ready_and_lo)
  ...
  typedef enum logic [4:0] {
    RESET_WAIT,
    TAGST_INIT, TAGST_DRAIN,
    WARMUP, WARMUP_GAP,
    EVICT, EVICT_GAP,
    RE_WARMUP, RE_WARMUP_GAP,
    WRITE, WRITE_GAP,
    READ, READ_GAP,
    SPM_WRITE, SPM_WRITE_GAP,
    SPM_READ, SPM_READ_GAP,
    IDLE, DONE
  } phase_e;
  ...
  // SPM way counter: 1..8 (number of ways enabled in current SPM round)
  integer spm_num_ways_r, spm_num_ways_n;

  // Active ways mask for current SPM round
  wire [ways_p-1:0] spm_ways_mask = (ways_p'(1) << spm_num_ways_r) - 1;
  ...
  // SPM read valid (1-cycle latency)
  ...
  always_comb begin
    ...
    case (phase_r)
      ...
      WARMUP: begin
        cache_pkt.opcode = SD; cache_pkt.addr = warmup_addr;
      ...
      READ_GAP: begin
        if (gap_cnt_r >= phase_gap_cycles_lp - 1) begin
          phase_n = SPM_WRITE; op_cnt_n = 0;
          spm_num_ways_n = 1;  // start from 1 way
        end
        else gap_cnt_n = gap_cnt_r + 1;
      end

      // ----- SPM WRITE (N ways simultaneously) -----
      SPM_WRITE: begin
        for (int w = 0; w < ways_p; w++) begin
          if (w < spm_num_ways_r) begin
            sp_en[w]     = 1'b1;
            sp_v[w]      = 1'b1;
            sp_w[w]      = 1'b1;
            sp_addr[w]   = op_cnt_r[0+:lg_sp_bank_els_lp];
            sp_wdata[w]  = lfsr_r ^ (64'(w) << 8);  // per-way unique data
            sp_w_mask[w] = '1;
          end
        end
      ...
      // ----- SPM READ (N ways simultaneously) -----
      SPM_READ: begin
        for (int w = 0; w < ways_p; w++) begin
          if (w < spm_num_ways_r) begin
            sp_en[w]   = 1'b1;
      ...
      SPM_READ_GAP: begin
        sp_en = spm_ways_mask;
        if (gap_cnt_r >= phase_gap_cycles_lp - 1) begin
          if (spm_num_ways_r < ways_p) begin
            // Next round: enable one more way
            spm_num_ways_n = spm_num_ways_r + 1;
            phase_n = SPM_WRITE; op_cnt_n = 0;
          end else begin
            phase_n = IDLE; gap_cnt_n = 0;
          end
        end
        else gap_cnt_n = gap_cnt_r + 1;
      end
  ...
  // -------------------------------------------------------
  // Operation type (for power annotation)
  // Encodes both phase and SPM way count
  // -------------------------------------------------------
  typedef enum logic [4:0] {
    OP_INIT,
    OP_WARMUP,
    OP_WRITE,
    OP_READ,
    OP_SPM_WR_1W, OP_SPM_WR_2W, OP_SPM_WR_3W, OP_SPM_WR_4W,
    OP_SPM_WR_5W, OP_SPM_WR_6W, OP_SPM_WR_7W, OP_SPM_WR_8W,
    OP_SPM_RD_1W, OP_SPM_RD_2W, OP_SPM_RD_3W, OP_SPM_RD_4W,
    OP_SPM_RD_5W, OP_SPM_RD_6W, OP_SPM_RD_7W, OP_SPM_RD_8W,
    OP_IDLE
  } op_type_e;
  ...
  always_comb begin
    case (phase_r)
      ...
      SPM_WRITE, SPM_WRITE_GAP:                    op_type = op_type_e'(OP_SPM_WR_1W + spm_num_ways_r - 1);
      SPM_READ, SPM_READ_GAP:                      op_type = op_type_e'(OP_SPM_RD_1W + spm_num_ways_r - 1);
  ...
  always_ff @(posedge clk) begin
    if (!reset) begin
      ...
      // SPM phase transitions
      if (phase_r != SPM_WRITE && phase_n == SPM_WRITE)
        $display("[PHASE_SPM_WR_%0dW_START] cycle=%0d  ops=%0d  ways=0..%0d",
          spm_num_ways_n, cycle_cnt, num_spm_ops_lp, spm_num_ways_n-1);
      if (phase_r == SPM_WRITE && phase_n == SPM_WRITE_GAP)
        $display("[PHASE_SPM_WR_%0dW_END]   cycle=%0d", spm_num_ways_r, cycle_cnt);
      if (phase_r == SPM_WRITE_GAP && phase_n == SPM_READ)
        $display("[PHASE_SPM_RD_%0dW_START] cycle=%0d  ops=%0d  ways=0..%0d",
          spm_num_ways_r, cycle_cnt, num_spm_ops_lp, spm_num_ways_r-1);
      if (phase_r == SPM_READ && phase_n == SPM_READ_GAP)
        $display("[PHASE_SPM_RD_%0dW_END]   cycle=%0d", spm_num_ways_r, cycle_cnt);

      if (phase_n == IDLE && phase_r == SPM_READ_GAP)
  ...
  // -------------------------------------------------------
  // Data correctness check - Cache (shadow memory)
  // -------------------------------------------------------
  ...
  always_ff @(posedge clk) begin
    if (reset) rd_check_idx_r <= 0;
    else if (phase_r == WRITE_GAP && phase_n == READ) rd_check_idx_r <= 0;
    else if (phase_r == READ && v_lo && yumi_li) rd_check_idx_r <= rd_check_idx_r + 1;
  end
  ...
  // -------------------------------------------------------
  // Data correctness check - SPM (shadow memory, per-way)
  // Check way 0 of each round (always enabled)
  // -------------------------------------------------------
  logic [data_width_p-1:0] spm_shadow_mem [sp_bank_els_lp-1:0];
  integer spm_error_cnt;

  // Record writes for way 0 (data = lfsr ^ 0)
  always_ff @(posedge clk) begin
    if (phase_r == SPM_WRITE)
      spm_shadow_mem[op_cnt_r % sp_bank_els_lp] <= lfsr_r;  // way 0 data = lfsr ^ 0
  end

  integer spm_check_idx_r;
  always_ff @(posedge clk) begin
    if (reset) spm_check_idx_r <= 0;
    else if (phase_r == SPM_WRITE_GAP && phase_n == SPM_READ) spm_check_idx_r <= 0;
    else if (spm_rd_valid_r) spm_check_idx_r <= spm_check_idx_r + 1;
  end

  always_ff @(posedge clk) begin
    if (reset) spm_error_cnt <= 0;
    else if (spm_rd_valid_r) begin
      if (sp_rdata[0] !== spm_shadow_mem[spm_check_idx_r % sp_bank_els_lp]) begin
        $error("[CHECK FAIL] SPM_RD_%0dW #%0d: way0 got=0x%016h exp=0x%016h cycle=%0d",
          spm_num_ways_r, spm_check_idx_r,
          sp_rdata[0], spm_shadow_mem[spm_check_idx_r % sp_bank_els_lp], cycle_cnt);
        spm_error_cnt <= spm_error_cnt + 1;
      end
    end
  end
  ...
  always_ff @(posedge clk) begin
    if (phase_r == SPM_READ && phase_n == SPM_READ_GAP)
      $display("[CHECK] SPM_RD_%0dW done: %0d errors out of %0d reads",
        spm_num_ways_r, spm_error_cnt, num_spm_ops_lp);
  end
